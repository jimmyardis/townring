/* ============================================================
   The Sumter Map — Voice + Data Integration
   ============================================================
   Connects the page to your Vapi assistant via the Web SDK.
   Client-side function calling — when the agent needs real data,
   it calls one of the tools below and weaves the answer into
   its response.
   ============================================================ */
...
const TOOLS = {
  ...
  fly_to_place({ place, name, target, location }) {
    ...
    return window.sumterMap?.flyTo?.(p) ?? { error: 'Map control not initialized.' };
  },
  ...
  set_metric({ metric }) {
    return window.sumterMap?.setMetric?.(metric) ?? { error: 'Map control not initialized.' };
  },

  scrub_year({ year }) {
    return window.sumterMap?.setYear?.(year) ?? { error: 'Map control not initialized.' };
  },

  toggle_layer({ layer }) {
    return window.sumterMap?.toggleLayer?.(layer) ?? { error: 'Map control not initialized.' };
  },

  reset_view() {
    return window.sumterMap?.reset?.() ?? { error: 'Map control not initialized.' };
  },
};
  ...
  if (isToolMessage && toolCallList && toolCallList.length > 0) {
    for (const call of toolCallList) {
      ...
      for (const fmt of responseFormats) {
        try { vapi.send(fmt); console.log(`  ↳ sent response format: ${fmt.type}`); }
        catch (e) { console.log(`  ↳ format ${fmt.type} failed:`, e?.message || e); }
      }
    }
    return;
  }
...
function showTranscript(role, text) {
  ...
  transcriptEl.innerHTML = `<span class="role">${icon}</span><span class="text">${escapeHtml(text)}</span>`;
  transcriptEl.classList.add('visible');
}

function hideTranscript() { transcriptEl.classList.remove('visible'); }
